...
export function RSVPTable({ data }: RSVPTableProps) {
  ...
  const getAttendanceBadge = (attendance: string) => {
    switch (attendance) {
      ...
      default:
        return <span className="text-stone-500 dark:text-stone-400">{attendance}</span>;
    }
  };
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          ...
          <div className="flex gap-2">
            <Button
              ...
              className={statusFilter === "all" ? "bg-amber-600 hover:bg-amber-700" : "border-stone-300 dark:border-stone-700"}
            >
              All
            </Button>
            <Button
              ...
              className={statusFilter === "yes" ? "bg-amber-600 hover:bg-amber-700" : "border-stone-300 dark:border-stone-700"}
            >
              Attending
            </Button>
            <Button
              ...
              className={statusFilter === "maybe" ? "bg-amber-600 hover:bg-amber-700" : "border-stone-300 dark:border-stone-700"}
            >
              Maybe
            </Button>
          </div>
        ...
        <Table>
          <TableHeader className="bg-stone-50 dark:bg-stone-900/50">
            <TableRow className="hover:bg-stone-50 dark:hover:bg-stone-900/50 border-stone-200 dark:border-stone-700">
              <TableHead className="text-stone-700 dark:text-stone-300 font-medium">Name</TableHead>
              <TableHead className="text-stone-700 dark:text-stone-300 font-medium">Email</TableHead>
              <TableHead className="text-stone-700 dark:text-stone-300 font-medium">Guests</TableHead>
              <TableHead className="text-stone-700 dark:text-stone-300 font-medium">Response</TableHead>
              <TableHead className="text-stone-700 dark:text-stone-300 font-medium">Date</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
                ...
                <TableCell colSpan={5} className="h-32 text-center">
                  <div className="flex flex-col items-center justify-center text-stone-500 dark:text-stone-400">
                    ...
                    <p className="text-sm mt-1">
                      {filter ? "Try adjusting your search or filter" : "No responses yet"}
                    </p>
                  </div>
                </TableCell>
              </TableRow>
      ...
      {filteredData.length > 0 && (
        <div className="flex flex-col sm:flex-row items-center justify-between mt-4 text-sm text-stone-600 dark:text-stone-400">
          <div>
            ...
            <span className="font-medium">{data.length}</span> responses
          </div>
          <Button 
            variant="outline" 
            size="sm"
